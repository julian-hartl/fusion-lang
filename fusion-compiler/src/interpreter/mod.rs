...
//     i64::from_le_bytes(value.try_into().expect(format!("Expected {} bytes, got {}", std::mem::size_of::<i64>(), value.len()).as_str()))
// }
//
...
//                     InstructionKind::Store { ptr: ptr, ty: primary } => {
//                         let value = self.interpret_primary(primary);
//                         let ty = match ptr {
//                             MemoryPointer::Variable(variable, ty) => {
//                                 self.state.save_var(*variable, ty.layout(self.scope).size, &value);
//                                 ty.clone()
//                             }
...
//                                 ty.clone()
//                             }
//                             MemoryPointer::Base(base, offset, ty) => {
//                                 let base: Ptr = self.get_address(base).try_into().unwrap();
//                                 let raw_ptr = base.add_offset(*offset);
//                                 assert_eq!(value.len(), ty.layout(self.scope).size, "Value size does not match type size");
//                                 self.state.store(&raw_ptr, &value);
//                                 ty.clone()
//                             }
//                         };
//                         (value, ty)
//                     }
//                     InstructionKind::Call { function_id: function_id, args: args } => {
...
//                             let argument_size = parameter.ty.layout(self.scope).size;
...
//                     }
//                     InstructionKind::Primary { value: primary } => {
//                         let value = self.interpret_primary(primary);
//                         (value, primary.ty())
//                     }
//                     InstructionKind::BinaryOp { operator: op, lhs: lhs, rhs: rhs, result_ty: ty } => {
...
//                         (value, ty.clone())
//                     }
//                     InstructionKind::UnaryOp { operator: op, operand: operand, result_type: ty } => {
...
//                             MemoryPointer::Primary(raw_ptr, ty) => {
//                                 let raw_ptr: Ptr = self.interpret_primary(raw_ptr).try_into().unwrap();
//                                 (self.state.get(&raw_ptr, ty.layout(self.scope).size).to_vec(), ty.clone())
//                             }
//                             MemoryPointer::Base(base, index, ty) => {
//                                 let base: Ptr = self.get_address(base).try_into().unwrap();
//                                 let raw_ptr = base.add_offset(*index);
//                                 (self.state.get(&raw_ptr, ty.layout(self.scope).size).to_vec(), ty.clone())
//                             }
//                         }
//                     }
//                     InstructionKind::GetAddress { store_into: expr } => {
//                         let value = self.get_address(expr);
//                         (value, expr.ty())
//                     }
...
//                         (value, ty.clone())
//                     }
//                     InstructionKind::Alloc { var: size, ty: ty } => {
//                         let ptr = self.state.alloc(*size);
//                         (ptr.as_bytes().to_vec(), ty.clone())
//                     }
//                 };
//                 if let Some(assign_to) = instruction.assign_to {
//                     let size = ty.layout(self.scope).size;
//                     self.state.save_var(assign_to, size, &return_value);
//                 }
//             }
...
//                 }
//                 TerminatorKind::If { condition: cond, then: then, else_: else_ } => {
...
//     }
//
//     fn get_address(&mut self, expr: &Primary) -> Vec<u8> {
