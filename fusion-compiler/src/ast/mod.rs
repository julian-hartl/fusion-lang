...
impl Ast {
    ...
    pub fn function_expression(&mut self, func_keyword: Token, parameters: Vec<FuncDeclParameter>, body: ExprId, return_type: Option<FunctionReturnTypeSyntax>) -> &Expr {
        self.expr_from_kind(ExprKind::Function(FuncExpr {
...
impl Expr {
    ...
    pub fn span(&self, ast: &Ast) -> TextSpan {
        match &self.kind {
            ...
            ExprKind::Call(expr) => {
                let identifier = ast.query_expr(expr.callee).span(ast);
            ...
            ExprKind::Function(FuncExpr { decl }) => {
                let fn_span = decl.func_keyword.span.clone();
                fn_span
            }
        }
    }
...
#[cfg(test)]
mod test {
    use crate::ast::{AssignExpr, Ast, BinaryExpr, BlockExpr, BoolExpr, CallExpr, Expr, ExprId, FuncExpr, FunctionDeclaration, IfExpr, LetStmt, NumberExpr, ParenthesizedExpr, ReturnStmt, Stmt, UnaryExpr, VarExpr, WhileStmt};
