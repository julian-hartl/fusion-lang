...
use crate::ast::{AssignExpr, Ast, BinaryExpr, BinOpKind, BlockExpr, BoolExpr, CallExpr, Expr, ExprId, FuncExpr, FunctionDeclaration, IfExpr, LetStmt, NumberExpr, ParenthesizedExpr, Stmt, UnaryExpr, UnOpKid, VarExpr, WhileStmt};
...
impl<'a> ASTEvaluator<'a> {
    ...
    fn expect_last_value(&self) -> Value {
        self.last_value.expect("Expected last value")
    }
}

impl<'a> ASTVisitor for ASTEvaluator<'a> {
    fn visit_func_expr(&mut self, ast: &mut Ast, function_expr: &FuncExpr, expr_id: ExprId) {
        let function_id = ast.query_expr(expr_id).ty.expect_function();
        self.last_value = Some(Value::Function(*function_id));
    }
    ...
    fn visit_call_expression(&mut self, ast: &mut Ast, call_expression: &CallExpr, expr: &Expr) {
        ...
        let function_idx = self.expect_last_value().expect_function();
        let function = &self.global_scope.functions[function_idx];
        ...
        self.push_frame();
        assert_eq!(arguments.len(), function.parameters.len());
        for (argument, param) in arguments.into_iter().zip(function.parameters.iter()) {
    ...
    fn visit_variable_expression(&mut self, ast: &mut Ast, var_expr: &VarExpr, expr: &Expr) {
        ...
        self.last_value = Some(*self.frames.get(&var_expr.variable_idx).unwrap_or_else(|| panic!("Variable {} '{}' not found",var_expr.variable_idx.as_index(), identifier)));
    }
    ...
    fn visit_unary_expression(&mut self, ast: &mut Ast, unary_expression: &UnaryExpr, expr: &Expr) {
        ...
        self.last_value = Some(Value::Number(
